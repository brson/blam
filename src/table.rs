...
pub type ColumnName = std::string::String;
...
pub struct Column {
    ...
    pub data: ColumnData,
    pub unique_key: bool,
    pub foreign_key: Option<(TableName, ColumnName)>,
}
...
pub struct JoinCriteria {
    ...
    pub self_column: ColumnName,
    pub other_column: ColumnName,
    pub other_table_rename: Option<TableName>,
}
...
impl Table {
    pub fn from_columns(name: TableName, columns: Vec<Column>) -> Table {
        let mut rows = None;
...
impl Table {
    ...
    pub fn join(&self, other: &Table, crit: JoinCriteria) -> Table {
    ...
    pub fn select(&self, crit: SelectCriteria) -> Table {
        let name = crit.new_table_name;
        let rows = self.rows;
